# [188. Best Time to Buy and Sell Stock IV](https://leetcode.com/problems/best-time-to-buy-and-sell-stock-iv/description/?envType=study-plan-v2&envId=top-interview-150) <Badge type="danger" text="Hard" />
...
class Solution {
    func maxProfit(_ k: Int, _ prices: [Int]) -> Int {
        ...
        var buys = Array(repeating: Int.min, count: k)
        var sells = Array(repeating: 0, count: k)
        for price in prices {
            for j in 0..<k {
                let prevSell = j == 0 ? 0 : sells[j-1]
                buys[j] = max(buys[j], prevSell - price)
                sells[j] = max(sells[j], buys[j] + price)
            }
        }
        return sells[k-1]
