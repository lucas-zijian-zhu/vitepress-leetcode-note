...
The key insight is to use **post-order traversal** (left ➝ right ➝ root) to find the LCA. At each node, we check if it's the LCA by examining the results from its left and right subtrees.
