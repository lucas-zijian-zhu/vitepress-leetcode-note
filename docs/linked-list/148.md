...
## 💡 Approach 1: Merge Sort (Top-down) — Easier to Follow

### Intuition

Use **fast/slow pointers** to find the middle, split the list in two, then recursively sort and merge. Classic divide-and-conquer: split then merge, more intuitive than bottom-up.

**Key Steps:**
1. **Split:** fast moves two steps, slow one step; when fast reaches the end, slow is at mid; set `prev.next = null` to cut the list.
2. **Recurse:** `sortList(left)` and `sortList(right)`.
3. **Merge:** merge two sorted lists (dummy + two pointers).

- **Time:** O(n log n)  
- **Space:** O(log n) recursion stack. Prefer this when O(1) space is not required; it is easier to understand.
...
function sortList(head: ListNode | null): ListNode | null {
    ...
    let slow = head, fast = head, prev: ListNode | null = head;
...
function mergeList(p: ListNode | null, q: ListNode | null): ListNode | null {
...
## 💡 Approach 2: Merge Sort (Bottom-up) — O(1) Space

### Intuition

To achieve **O(1) space** (Follow up), use **bottom-up**: no recursion; split by lengths 1, 2, 4, 8, ... and merge adjacent pairs. Conceptually start with each node as a sorted run, then merge into longer sorted runs in each pass.
...
- **Time Complexity:** O(n log n) — log n divide/merge rounds, each merge is O(n).
- **Space Complexity:**
  - **Top-down:** O(log n) recursion stack.
  - **Bottom-up:** O(1), satisfying the Follow up.