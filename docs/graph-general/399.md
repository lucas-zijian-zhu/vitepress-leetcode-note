...
## 🚀 Approach: DFS/BFS Graph Traversal (Recommended)
...
### Method 1: DFS/BFS Graph Traversal (Recommended)
- **Time Complexity:** O((E + Q) * V) in worst case (each query might traverse the entire graph)
- **Space Complexity:** O(E + V) for adjacency list + O(V) for recursion stack
- **Query Time:** O(V + E) per query (can be slow for large graphs)

### Method 2: Union-Find (Alternative)
- **Time Complexity:** O((E + Q) * α(V)), where α is the inverse Ackermann function (practically constant ≈ 4)
- **Space Complexity:** O(V) for parent and weight maps
- **Query Time:** O(α(V)) per query (extremely fast)
